...
class Datainfo:
    ...
    def getnextdata(df,symbol):
        ...
        features_train = df_20d[:680]
        labels_train = df_20d['close'].shift(-1)[:680]     # 回归问题的标签就是预测的就是股价，下一天的收盘价就是前一天的标签；
        features_test = df_20d[680:]
        labels_test = df_20d['close'].shift(-1)[680:]
        ...
        #获取预期下个整点的值
        print(df2['Predicted Next Close'].tail(1).values[0] ,df2['Current Close'].tail(1).values[0],df2['Predicted Next Close'].tail(1).values[0] > df2['Current Close'].tail(1).values[0])
        print(df2.tail(5))
    ...
    class mywindows_multiprocessing ():
        ...
        def run():
            ...
            p2 = multiprocessing.Process(target = sch.okex1M_buy)
            p3 = multiprocessing.Process(target = sch.okex3M_buy)
            p4 = multiprocessing.Process(target = sch.okex5M_buy)
            p5 = multiprocessing.Process(target = sch.okex15M_buy)
            p6 = multiprocessing.Process(target = sch.okex60M_buy)
            p7 = multiprocessing.Process(target = sch.okex240M_buy)

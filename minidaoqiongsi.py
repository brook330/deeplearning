...
class NQ_Datainfo():
    ...
    def final_NQ_1M(self):
        if(self.timepass()):
            df = self.getdatainfo_minute(1,'NQ')
            self.buyinfo(df,'NQ',1)

    def final_YM_1M(self):
        if(self.timepass()):
            df = self.getdatainfo_minute(1,'YM')
            self.buyinfo(df,'YM',1)

    def final_HSI_1M(self):
        if(self.time_hsi_quantian()):
            df = self.getdatainfo_minute(1,'HSI')
            self.buyinfo(df,'HSI',1)
...
class RuntimeScheduler:
    ...
    def get_final_NQ_1M_job(self):

        
        nqdata = NQ_Datainfo()
        scheduler = BlockingScheduler()
        scheduler.add_job((nqdata.final_NQ_1M), 'cron', minute='*/1')
    ...
    def get_final_YM_1M_job(self):

        
        nqdata = NQ_Datainfo()
        scheduler = BlockingScheduler()
        scheduler.add_job((nqdata.final_YM_1M), 'cron', minute='*/1')
        print(scheduler.get_jobs())
        try:
            scheduler.start()
        except KeyboardInterrupt:
    ...
    def get_final_HSI_1M_job(self):

        
        nqdata = NQ_Datainfo()
        scheduler = BlockingScheduler()
        scheduler.add_job((nqdata.final_HSI_1M), 'cron', minute='*/1')
        print(scheduler.get_jobs())
        try:
            scheduler.start()
        except KeyboardInterrupt:
...
if __name__ == '__main__':
    ...
    #定义进程
    p1 =multiprocessing.Process(target = dingzhi.get_final_YM_1M_job)
    p2 =multiprocessing.Process(target = dingzhi.get_final_YM_5M_job)
    p3 =multiprocessing.Process(target = dingzhi.get_final_YM_15M_job)
    p4 =multiprocessing.Process(target = dingzhi.get_final_NQ_1M_job)
    p5 =multiprocessing.Process(target = dingzhi.get_final_NQ_5M_job)
    p6 =multiprocessing.Process(target = dingzhi.get_final_NQ_15M_job)
    p7 =multiprocessing.Process(target = dingzhi.get_final_HSI_1M_job)
    p8 =multiprocessing.Process(target = dingzhi.get_final_HSI_5M_job)
    p9 =multiprocessing.Process(target = dingzhi.get_final_HSI_15M_job)

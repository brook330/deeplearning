...
class Datainfo:
    ...
    def isbuy():
        ...
        result = df['takerBuyVolValue'].values[-1]>df['takerBuyVolValue'].values[-2] and df['takerBuyVol'].values[-1]>df['takerBuyVol'].values[-2] and df['plot'].values[-1]>1 and df['plot'].values[-1] > df['plot'].values[-2]

        Datainfo.saveinfo('获取数据完毕。。。   判断为： -->>'+str(result)+'   -->>我们是守护者，也是一群时刻对抗危险和疯狂的可怜虫 ！^_^')

        #保存所有数据
        path = Path(f'./datas/okex/eth')
        df = pd.DataFrame(columns=['timestamp']).set_index('timestamp')
        for file in path.iterdir():
            df0 = pd.read_csv(file).set_index('timestamp')
            df = df.merge(df0, left_index=True, right_index=True, how='outer')
        df.to_csv(f'./datas/okex/eth/ethusd_final.csv',index = False)
    ...
    class windowsshow():
        ...
        def okex5M_buy(self):
            ...
            scheduler.add_job((self.getdatainfo), 'cron', minute='*/6')

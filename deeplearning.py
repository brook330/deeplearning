...
class Datainfo:
    ...
    def isbuy():
        ...
        'cookie': 'locale=zh_CN; _gcl_au=1.1.1849415495.1622314481; _ga=GA1.2.1506507962.'+tt+'; _gid=GA1.2.256681666.'+tt+'; amp_56bf9d=gqC_GMDGl4q5Tk-BJhT-oP...1f6snl1p6.1f6snloll.0.0.0',
        }
        ...
        df2 = pd.DataFrame(eval(json.dumps(response2.json()))['data'])
        df['volumes'] = df2['volumes']

        df.to_csv(f'./datas/okex/eth/ethusd.csv',index = False)
        df = pd.read_csv(f'./datas/okex/eth/ethusd.csv')
        result = df['takerBuyVolValue'].values[-1]>df['takerBuyVolValue'].values[-2] and df['takerBuyVol'].values[-1]>df['takerBuyVol'].values[-2] and df['plot'].values[-1]>1 and df['plot'].values[-1] > df['plot'].values[-2] and df['volumes'].values[-1] > df['volumes'].values[-2]

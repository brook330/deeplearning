...
class Datainfo:
    ...
    def isbuy():
        ...
        headers = {
            ...
            'devid': 'fdc2e915-a1cc-4545-a6b7-3d6f1b9bfe31',
            ...
            'cookie': 'locale=zh_CN; _gcl_au=1.1.626599872.'+tt+'; _ga=GA1.2.387735640.'+tt+'; _gid=GA1.2.935842369.'+tt+'; _gat_UA-35324627-3=1; amp_56bf9d=GCEjDGiPX4Z2b4rPFjcEzs...1f6cplffm.1f6cplffm.1.0.1',
        }
        ...
        response = r.get('https://www.okex.com/v2/perpetual/pc/public/calculate/fundingRate', headers=headers, params=params,timeout=1)
        ...
        if response.cookies.get_dict(): #保持cookie有效 
            ...
            s.get(url='https://www.okex.com/v2/perpetual/pc/public/calculate/fundingRate?t='+ttt+'&interval=5&contract=ETH-USDT-SWAP')

        data = response.json()['data']['fundingRate']
        c_data = json.dumps(data)
        c_data = eval(c_data)

        final = c_data[-1]>c_data[-2]+0.00003

        Datainfo.saveinfo('获取数据完毕。。。\n判断为：\n'+str(final))
    ...
    def orderbuy(api_key, secret_key, passphrase, flag):
        ...
        result = tradeAPI.place_algo_order('ETH-USD-SWAP', 'cross', 'sell', ordType='conditional',
                                            sz='1',posSide='long', tpTriggerPx=str(float(lastprice)+10), tpOrdPx=str(float(lastprice)+9))
        Datainfo.saveinfo('设置止盈完毕。。。'+str(float(lastprice)+10))

        sendtext = '100倍杠杆，全仓委托：ETH-USD-SWAP1笔，价格是'+str(lastprice)+'，设置止盈完毕。。。'+str(float(lastprice)+10)

...
class Datainfo:
    ...
    def orderbuy(api_key, secret_key, passphrase, flag,symbol):
        ...
        result = tradeAPI.place_multiple_orders([
             {'instId': symbol.upper()+'-USD-SWAP', 'tdMode': 'cross', 'side': 'buy', 'ordType': 'market', 'sz': '300',
              'posSide': 'long',
        ...
        result = tradeAPI.place_algo_order(symbol.upper()+'-USD-SWAP', 'cross', 'sell', ordType='conditional',
                                            sz='300',posSide='long', tpTriggerPx=str(float(lastprice)*1.002), tpOrdPx=str(float(lastprice)*1.002))
        #Datainfo.saveinfo(str(datetime.now())+'设置止盈完毕。。。'+str(float(lastprice)+50))


        sendtext = '买入'+symbol.upper()+'-USD-SWAP -->> 300笔，价格是'+str(lastprice)+'，设置止盈完毕。。。'+str(float(lastprice)*1.002)
        Datainfo.save_finalinfo('买入价格是--》》'+str(lastprice)+'，设置止盈完毕。。。'+str(float(lastprice)*1.002))
    ...
    class mywindows_multiprocessing ():
        ...
        def run():
            ...
            p2 = multiprocessing.Process(target = sch.okex60M_buy)
